...
const cheapestByCategory = (products) => 
    products.reduce((acc, curr) => {
        if (!acc[curr.category] || acc[curr.category].price > curr.price) {
...
const totalInventoryValue = (products) => 
    products.reduce((acc, curr) => acc + (curr.price * curr.stock), 0);
...
const averageRating = (products) => 
    products.length === 0 ? 0 : products.reduce((acc, p) => acc + p.rating, 0) / products.length;
